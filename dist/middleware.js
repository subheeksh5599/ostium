...
export class MandateMiddleware {
    ...
    getActiveMandates() {
        const now = new Date();
        return Array.from(this.mandates.values()).filter((m) => new Date(m.mandate.expiresAt) > now);
    }
    checkAction(mandateId, action) {
        ...
        if (action.ticker && action.amount) {
            ...
            const tokenLimit = signed.mandate.tokens.find((t) => t.ticker.toLowerCase() === action.ticker.toLowerCase());
    ...
    computeExecutionDisplay(action) {
        const lines = [];
        lines.push("╔═══════════════════════════════════╗");
        ...
        if (action.targetAddress) {
            const short = action.targetAddress.slice(0, 10) + "..." + action.targetAddress.slice(-6);
    ...
    computeSpeculosButtonSequence(mandate, action) {
