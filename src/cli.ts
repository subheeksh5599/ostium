...
import { AgentSDK, type AgentCredential } from "./agent.js";
...
import {
  ...
  type SignedMandate,
  type MandateAction,
  type MandateTokenLimit,
  type MandateProtocol,
} from "./mandate.js";
...
function ensureDir(): void {
  if (!existsSync(MANDATE_DIR)) mkdirSync(MANDATE_DIR, { recursive: true });
}

function loadCredentials(): AgentCredential[] {
  ensureDir();
  if (!existsSync(CREDENTIALS_FILE)) return [];
  return JSON.parse(readFileSync(CREDENTIALS_FILE, "utf-8"));
}

function saveCredentials(creds: AgentCredential[]): void {
  ensureDir();
  writeFileSync(CREDENTIALS_FILE, JSON.stringify(creds, null, 2));
}

function loadMandates(): SignedMandate[] {
  ensureDir();
  if (!existsSync(MANDATES_FILE)) return [];
  return JSON.parse(readFileSync(MANDATES_FILE, "utf-8"));
}

function saveMandates(mandates: SignedMandate[]): void {
  ensureDir();
...
program
  ...
  .action(async (opts) => {
    ...
    const tokens: MandateTokenLimit[] = opts.tokens.split(",").map((t: string) => {
    ...
    const protocols: MandateProtocol[] = opts.protocols
      .split(",")
      .map((p: string) => ({ name: p.trim() }));

    const agent = new AgentSDK({
      agentId: opts.agentId,
      agentPubkey: agentConfig?.agentPubkey ?? `pk:${Date.now()}`,
      network: (opts.network as Mandate["network"]) ?? "solana:devnet",
    ...
    if (opts.sign) {
      ...
      tokens.forEach((token) => {
        console.log(
          `  ${chalk.cyan(token.ticker.padEnd(8))} per-tx: ${chalk.yellow(String(token.perTxLimit).padEnd(8))} daily: ${chalk.yellow(String(token.dailyLimit))}`
        );
      });
      console.log();
      console.log(
        `  Protocols:   ${protocols.map((p) => chalk.magenta(p.name)).join(", ")}`
      );
...
program
  ...
  .action(async (opts) => {
    ...
    if (result.status === "approved") {
      ...
      if (opts.to) {
        console.log(
          chalk.dim(`  To:        ${opts.to.slice(0, 12)}...${opts.to.slice(-8)}`)
        );
      }
      console.log(chalk.dim(`  Amount:    ${opts.amount} ${opts.ticker}`));
      if (opts.protocol) console.log(chalk.dim(`  Protocol:  ${opts.protocol}`));
    } else {
    ...
    if (spendSummary.length > 0) {
      ...
      spendSummary.forEach((s) => {
        ...
        const color =
          s.percentUsed > 80 ? chalk.red : s.percentUsed > 50 ? chalk.yellow : chalk.green;
        console.log(
          `  ${chalk.cyan(s.ticker.padEnd(8))} ${color(bar)} ${chalk.dim(`${s.spent}/${s.limit}`)}`
        );
      });
    }
...
program
  ...
  .action(async (mandateId: string) => {
    ...
    printRow(
      "Signed By",
      signed.signedBy === "speculos" ? chalk.green(signed.signedBy) : chalk.dim(signed.signedBy)
    );
    ...
    m.tokens.forEach((t) => {
      console.log(
        `  ${chalk.cyan(t.ticker.padEnd(8))} per-tx: ${chalk.yellow(String(t.perTxLimit).padStart(6))}   daily: ${chalk.yellow(String(t.dailyLimit).padStart(6))}`
      );
    });
...
program
  ...
  .action(async (opts) => {
    ...
    const filtered = opts.mandateId
      ? logs.filter((l: any) => l.mandateId === opts.mandateId)
      : logs;
    ...
    limited.forEach((log: any) => {
      ...
      console.log(
        `  ${statusIcon} ${chalk.dim(time)}  ${chalk.cyan(log.mandateId.slice(0, 8))}  ${
          log.ticker?.padEnd(8) ?? ""
        } ${String(log.amount ?? "").padEnd(6)}  ${chalk.bold(log.status === "approved" ? chalk.green("OK") : chalk.red("BLOCKED"))}`
      );
      if (log.reason) {
...
function renderBar(percent: number, width: number): string {
  ...
  let bar = "";
  if (percent > 80) bar = chalk.red("█".repeat(filled)) + chalk.dim("░".repeat(empty));
  else if (percent > 50) bar = chalk.yellow("█".repeat(filled)) + chalk.dim("░".repeat(empty));
  else bar = chalk.green("█".repeat(filled)) + chalk.dim("░".repeat(empty));

  return bar;
}

function displaySigningFlow(mandate: Mandate): void {
  console.log(chalk.dim("\n  ┌──────────────────────────────────┐"));
  ...
  console.log(chalk.dim("  │                                  │"));
  console.log(
    chalk.dim("  │  ") + chalk.cyan("Agent:") + ` ${mandate.agentId.padEnd(25)}` + chalk.dim("│")
  );
  console.log(
    chalk.dim("  │  ") +
      chalk.cyan("Network:") +
...
function loadAgentConfig(agentId: string): { agentId: string; agentPubkey: string; network: string; derivationPath: string } | null {
...
async function renderDashboard(focusMandateId?: string): Promise<void> {
  const mandates = loadMandates();
  const now = new Date();
  ...
  active.forEach((signed) => {
    ...
    m.tokens.forEach((t) => {
      console.log(
        chalk.dim(`         ${t.ticker.padEnd(8)} per-tx: ${String(t.perTxLimit).padEnd(6)} daily: ${String(t.dailyLimit).padEnd(6)}  ${m.protocols.map(p => p.name).join(", ")}`)
      );
    });
    console.log();
  });
...
function renderRemainingBar(remaining: number, total: number): string {
  const width = 20;
  const filled = Math.round(((total - remaining) / total) * width);
  const empty = width - filled;

  if (remaining <= 2) return chalk.red("█".repeat(filled) + "░".repeat(empty));
  if (remaining <= 6) return chalk.yellow("█".repeat(filled) + "░".repeat(empty));
...
async function runDemo(): Promise<void> {
  ...
  logs.forEach((log) => {
    ...
    print(
      `  ${icon} ${D(log.timestamp.slice(0, 19).replace("T", " "))}  ${log.ticker ?? "  "}  ${String(log.amount).padEnd(4)}  ${status}  ${log.reason ? R(log.reason) : ""}`
    );
  });
